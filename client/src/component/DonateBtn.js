  ...
  return (
    <>
      <div
        ...
        p-3  uppercase text-white text-xl active:scale-105 duration-200 ease-in-out
        w-1/5 text-center "
...
function DonateTemplate({ id }) {
  const { DonateProject, setDonateProject, marketContract, setShowAlert } =
    useGlobalContext();
  const [donateAmt, setDonateAmt] = useState("");

  const Donate = async () => {
    try {
      const DonateTxn = await marketContract.donate(id, {
        value: ethers.utils.parseUnits(donateAmt.toString(), "ether"),
      });
      ...
      console.log();
      setShowAlert({
        status: true, //execution reverted
        message: `donate error ${error?.reason?.slice(
    ...
    >
      <div className="bg-PrimaryDark/90 CarsouelStyle rounded-xl md:w-3/4 lg:w-1/2 p-6 space-y-5">
        <div className="flex flex-row items-center justify-between pb-12 text-blue-400">
          ...
          <XMarkIcon
            onClick={() => setDonateProject("scale-0")}
