...
function Character() {
  ...
  const { marketContract, showAlert, setShowAlert } = useGlobalContext();
  ...
  const CharacterCreate = async (e) => {
    e.preventDefault();
    try {
      const CreateTxn = await marketContract.CharacterMake(
        name,
        description,
        imageUrl
      );
      ...
      setShowAlert({
        status: true,
        message: `${name} Character Created `,
      });
      reset();
      navigate("/");
    } catch (error) {
  ...
  return (
    <div className="h-screen bg-PrimaryDark flex items-center justify-center bg-opacity-80 ">
      <div className="bg-[#1d314b] CreateShadow rounded-xl w-3/4 p-6 ">
        <h1 className="text-3xl text-center text-white uppercase py-8">
          Create Avatar
        </h1>
        <form
          onSubmit={CharacterCreate}
